...
local current_wp = 0
--id sensor to use in ALING state
local id_align = 0
local max_d_in_search = 0
...
local dist_callback= function(d1, d2, d3, d4, d5, d6)
  ...
  --if current_wp == 0 then return end

  if h_state == H_SEARCH then
    local cur_max = max(norm_d)
    if cur_max > max_d_in_search then
      cur_wdot = W_ROTATE
      xdot = 0max
      ydot = 0
      omni.drive(xdot,ydot,cur_wdot)
      max_d_in_search = cur_max
    else
      id_align = indexsort(norm_d)
      if (id_align > 0) and (norm_d[id_align] >= max_d_in_search) then
        a_state = STOP
        h_state = H_ALIGN
        cur_wdot = 0
        xdot = 0
  ...
  elseif h_state == H_ALIGN then
    ...
    elseif a_state == PAN then
      -- print('debug d_last: ', d_last, 'id_align: ', id_align)
  ...
  elseif h_state == H_GO then
    ...
    -- if ((d_last - norm_d[id_align]) < TOL_APROACH) or norm_d[id_align] == 0 then
      ...
      foo = {0, 0, 0, 0, 0, 0}
      print(norm_d[id_align])
      foo[id_align] = line(1, 0.2, 0, 0.05, norm_d[id_align])
...
local time = 0
while true do
  -- print('hz: ', tic/time, 'st:', h_state[1], xdot, ydot, w, '-', 'dist(act_d):', table.unpack(act_d))
