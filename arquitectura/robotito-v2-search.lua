...
local dist_callback= function(d1, d2, d3, d4, d5, d6)
  ...
  for i = 1,N_SENSORS do
    ...
    last_d[i] = act_d[i] -- do not apply filters
    
    if last_d[i]<dmin or last_d[i]>dmax or (not MASK_ON_SENSORS[i]) then
      d[i]=0
      for j = 0, 3 do
  ...
  if d[1] == 0 then
      if cur_wdot == 0 then
        max_tics = INC_TIC_ARC_SEARCH
        cur_wdot = W_ROTATE
        cur_tics = 0
      end
      if cur_tics >= max_tics then
        max_tics = max_tics + INC_TIC_ARC_SEARCH
        cur_tics = 0
        cur_wdot = -1 * cur_wdot
      end
      cur_tics = cur_tics + 1
  ...
  if (max_tics >= INC_TIC_ARC_SEARCH * MAX_SEARCH_DIR_CHANGE) then -- seach timeout
    state = NEAR
    -- printprint("search timeout")
  end
