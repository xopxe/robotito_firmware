...
local TIMEOUT_SEARCH_60 = 2.0
local min_init = 9000
local last_min = min_init
local measures = {min_init,min_init,min_init,min_init,min_init, min_init}
...
local t_init_to_search
local t_end_debug



local init_s = ahsm.state{}

local search_farest_object_s = ahsm.state{
  entry=function() set_w(ROTATION_SPEED) end,
  doo=function()
    for i = 1,6 do
      if (robot.background.actual_dist[i] < measures[i]) then
        measures[i] = robot.background.actual_dist[i]
      end
    end
  end,
  exit=function()t_izq_no_to_der.timeout = TIMEOUT_INIT; set_w(0.0); print(table.unpack(measures))end,
}
...
local izq_to_der_s = ahsm.state{doo=function()
    if (robot.background.actual_dist[robot.background.id_align] > 0) then
...
local buscar_min_s = ahsm.state{
  ...
  doo=function()
    if ((robot.background.actual_dist[robot.background.id_align] ~= 0) and (robot.background.actual_dist[robot.background.id_align] < last_min)) then
      last_min = robot.background.actual_dist[robot.background.id_align]
    end
    return true
  end
}
...
local ajuste_s = ahsm.state{entry=invert_w,
  doo=function()
    if ((robot.background.actual_dist[robot.background.id_align] <= last_min + DELTA) and (robot.background.actual_dist[robot.background.id_align] >= DELTA)) then
...
t_init_to_search = ahsm.transition{
  src= init_s,
  tgt= search_farest_object_s,
  events = {init_s.EV_DONE}
}

t_end_debug = ahsm.transition{
  src= search_farest_object_s,
  tgt= going_s,
  timeout = TIMEOUT_SEARCH_60,
}
...
fsm = ahsm.state {
  ...
  states = {  init = init_s,
    ...
    search_farest_object = search_farest_object_s,
